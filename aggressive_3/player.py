...
class Player(Bot):
    ...
    def get_actions(self, game_state, round_state, active):
        ...
        for i in range(NUM_BOARDS-1, -1, -1):
            ...
            # round of active play
                ...
                # stresort_allocs_with_strengthrength = self.hole_strengths[i]
                ...
                if street < 3: #pre-flop
                    raise_amount = int(my_pips[i] + board_cont_cost + a * 0.4 * (pot_total + board_cont_cost)) #play a little conservatively pre-flop
                else:
                    raise_amount = int(my_pips[i] + board_cont_cost + a * 0.75 * (pot_total + board_cont_cost)) #raise the stakes deeper into the game
                ...
                # can't raise as much as we wanted, then raise as much as possible as least
                elif RaiseAction in legal_actions[i]:
                    commit_action = RaiseAction(my_stack - net_cost)
