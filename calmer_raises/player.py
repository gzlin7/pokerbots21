...
class Player(Bot):
    ...
    def get_actions(self, game_state, round_state, active):
        ...
        strengths = [self.calculate_strength(self.board_allocations[i], [card for card in board_cards[i] if card], 100) for i in range(NUM_BOARDS)]
        order = [i[0] for i in sorted(enumerate(strengths), key=lambda x:-x[1])]
        for i in order:
