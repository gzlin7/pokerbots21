    ...
    def get_actions(self, game_state, round_state, active):
        ...
        for i in range(NUM_BOARDS):
            ...
            # round of active play
                ...
                # a = 1.15
                #
                # # raise more for stronger bets
                # # same logic regardless of street, since strength takes into account
                # raise_amount = int(my_pips[i] + board_cont_cost + a * strength * (pot_total + board_cont_cost))
                #

                if street < 3:  # pre-flop
                    raise_amount = int(my_pips[i] + board_cont_cost + 0.4 * (
                                pot_total + board_cont_cost))  # play a little conservatively pre-flop
                else:
                    raise_amount = int(my_pips[i] + board_cont_cost + 0.75 * (
                ...
                elif RaiseAction in legal_actions[i] and (min_raise <= my_stack - net_cost): #raise if we can and if we can afford it
